...
const DashboardTopbar = ({ sidebarOpen, setSidebarOpen, darkMode, setDarkMode }) => {
  const { user, logout, balanceUSD } = useAuth();
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative" ref={currencyRef}>
            <button
              ...
              className="inline-flex items-center rounded-full bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white select-none"
            >
            ...
            {currencyOpen && (
              <div className="absolute right-0 mt-2 w-36 bg-white border border-slate-200 rounded shadow-md z-50">
                <button
                  onClick={() => { setCurrency('BDT'); setCurrencyOpen(false); }}
                  className={`w-full text-left px-3 py-2 text-sm ${currency === 'BDT' ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50'}`}
                >BDT ৳</button>
                <button
                  onClick={() => { setCurrency('USD'); setCurrencyOpen(false); }}
                  className={`w-full text-left px-3 py-2 text-sm ${currency === 'USD' ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50'}`}
                >USD $</button>
              </div>
            )}
          </div>
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-[13px] font-semibold text-slate-800 leading-snug">
                      ...
                      {(() => {
                        const formatted = (() => {
                          try {
                            return formatCurrency(balanceUSD, currency);
                          } catch (e) {
                            return currency === 'BDT' ? `৳ 0.00` : `$0.00`;
                          }
                        })();
                        return <p className="text-[11px] text-slate-500 leading-snug">{formatted}</p>;
                      })()}
                  </div>
                </div>
